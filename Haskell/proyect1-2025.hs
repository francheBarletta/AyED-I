...
arregloPositivo (x : xs)
  | x >= 0 = arregloPositivo xs
  | otherwise = False

arreglosIguales :: Int -> [Int] -> Bool
arreglosIguales n [] = False
arreglosIguales n (x : xs)
  | (x == n) = True
  | otherwise = arreglosIguales n xs

listasIguales :: [Int] -> Bool
listasIguales [] = True
listasIguales (x : y : xs)
  | (x == y) = listasIguales xs
  | otherwise = False
...
multiplicaPrimos [] = 1
multiplicaPrimos (x : xs)
  | esPrimo x = x * multiplicaPrimos xs
